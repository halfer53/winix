        ...
        if(child->parent == parent->proc_nr){
            ...
            **/
            ...
            if( (child->flags & ZOMBIE) ||
                (options & WUNTRACED && child->state & STOPPING) ){
...
int check_waiting(struct proc* who){
    ...
    foreach_proc(mp){
        ...
        if(who->parent == mp->proc_nr && mp->state & WAITING){
            pid_t pid = mp->wpid;
            if( (pid > 0 && pid == who->pid) ||
                (pid < -1 && -pid == who->procgrp) ||
                (pid == 0 && mp->procgrp == who->procgrp) ||
                (pid == -1)){
    ...
    //if the current process is zombie, and an arbitary parent process
    //is waiting for this process, release zombie
    if(children && who->flags & ZOMBIE)
        release_zombie(who);
    return children;
}
