...
void test_given_has_next_block_when_exceed_max_should_return_false(){
    struct zone_iterator iter;
    struct device* dev = get_dev(ROOT_DEV);
    struct inode* root = get_inode(ROOT_INODE_NUM, dev);
    ...
    bool result = has_next_block(&iter);
    assert(result == false);
}

void test_given_has_next_block_when_have_zone_should_return_true(){
    struct zone_iterator iter;
    struct device* dev = get_dev(ROOT_DEV);
    struct inode* root = get_inode(ROOT_INODE_NUM, dev);
    int ret = init_zone_iterator(&iter, root, 0);
...
void test_given_has_next_block_when_no_zone_should_return_false(){
    struct zone_iterator iter;
