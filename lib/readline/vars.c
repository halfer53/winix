...
int rl_eof_found = 0; // This is a flag that is set to 1 when readline() has found EOF on the input stream. It is useful for applications that want to call readline() multiple times in a loop, and do something else in between calls. The application should set this to 0 before calling readline() again.
